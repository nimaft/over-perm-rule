...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if configuration_item['resourceType'] == "AWS::IAM::Role":
        role_policies = iam_client.list_role_policies(RoleName=configuration_item['resourceName'])

        while True:

            for policy in role_policies['PolicyNames']:
                permissions = iam_client.get_role_policy(
                                RoleName=configuration_item['resourceName'],
                                PolicyName=policy
                            )
                for statement in permissions['PolicyDocument']["Statement"]:
                    if "sts:AssumeRole" in statement['Action'] and statement['Resource'][-1] == "*":
                        return 'NON_COMPLIANT'
            
            if "Marker" in role_policies:
                role_policies = iam_client.list_role_policies(RoleName=configuration_item['resourceName'],Marker=role_policies['Marker'])
            else:
                break

    if configuration_item['resourceType'] == "AWS::IAM::Policy": 
        permissions = iam_client.get_policy_version(
                        PolicyArn=configuration_item['configuration']['arn'],
                        VersionId=configuration_item['configuration']['defaultVersionId']
                    )
        for statement in permissions['PolicyVersion']['Document']['Statement']:
            if "sts:AssumeRole" in statement['Action'] and statement['Resource'][-1] == "*":
                return 'NON_COMPLIANT'
